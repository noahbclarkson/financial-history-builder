...
pub struct AccountingBalancer<'a> {
    history: &'a SparseFinancialHistory,
}

impl<'a> AccountingBalancer<'a> {
    pub fn new(history: &'a SparseFinancialHistory) -> Self {
        Self { history }
    }
    ...
    fn find_or_create_plug_account(
        ...
        for account in &self.history.accounts {
        ...
        // 4. String matching fallback (for generated accounts not in original history)
    ...
    fn calculate_balances(
        ...
        for (name, series) in dense_data.iter() {
            ...
            if let Some(&value) = series.get(&date) {
                if let Some(account) = self.history.accounts.iter().find(|a| a.name == *name) {
...
pub fn enforce_accounting_equation(
    ...
    let balancer = AccountingBalancer::new(history);
...
mod tests {
    ...
    fn test_enforce_accounting_equation() {
        ...
        enforce_accounting_equation(&history, &mut dense_data).unwrap();

        let result = verify_accounting_equation(&history, &dense_data, 0.01);
        assert!(result.is_ok());
    }
